...
public class RegistrarPersonaEstudiante implements Controller {
  ...
  public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	(!"".equals(sDireccion)) && (!"".equals(sId_tipo_estado_civil)) && (!"".equals(sId_tipo_sexo))  && (!"".equals(sId_tipo_empresa_telefonica)) && (!"".equals(sFec_inscripcion)))
   {
	...
	datoPst.setId_postulante(iId_postulante);
	datoPst.setId_estado("E"); //Regis
	datoPst.setUlt_usuario(cliente.getId_usuario()); 
	int iResCambio = this.mi.setPstModificarEstadoPostulante(datoPst);
        //Crear un tramite
        ...
        if (iId_tramite == 0) {
          return new ModelAndView("Error","mensaje","El trámite no se creó");
        }
	...
	System.out.println("El id_estudiante entrando -->"+Integer.toString(buscarEst.getId_estudiante()) );
        buscarEst= this.mi.getEstBuscarEstudiantePrs(buscarEst);
        modelo.put("buscarEst",buscarEst);
	System.out.println("El id_estudiante salida encontradoo -->"+Integer.toString(buscarEst.getId_estudiante()) );
